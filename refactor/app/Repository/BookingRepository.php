...
class BookingRepository extends BaseRepository
{
        ...
        if ($jobs) {
            ...
            $noramlJobs = collect($noramlJobs)
                ->each(function ($item, $key) use ($user_id) {
                    $item["usercheck"] = $this->model->checkParticularJob(
                        $user_id,
                        $item
                    );
                })
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($cuser) {
            if ($cuser->is("customer")) {
                // Retrieve jobs for the customer
                $jobs = $cuser
                    ->jobs()
                    ->with(
                        "user.userMeta",
                ...
                // Retrieve translator jobs using a custom method
                $jobs_ids = $this->model->getTranslatorJobsHistoric(
                    $cuser->id,
                    "historic",
                    $page
                );
        ...
        return abort(404); // You can change this to return an appropriate response for your application
    }
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env("CUSTOMER_ROLE_ID")) {
            ...
            $data["customer_phone_type"] = isset($data["customer_phone_type"])
                ? "yes"
                : "no";
            $data["customer_physical_type"] = isset(
                $data["customer_physical_type"]
            )
                ? "yes"
                : "no";
            ...
            // Set gender and certified based on job_for array
            $this->setGenderAndCertified($data);

            // Set job type based on consumer type
            $data["job_type"] = $this->getJobType($consumer_type);

            // Set created_at and will_expire_at fields
    ...
    private function setGenderAndCertified(&$data)
    {
        ...
        if (in_array("normal", $jobFor) && in_array("certified", $jobFor)) {
            ...
            in_array("normal", $jobFor) &&
            in_array("certified_in_law", $jobFor)
        ) {
            $data["certified"] = "n_law";
        } elseif (
            in_array("normal", $jobFor) &&
            in_array("certified_in_helth", $jobFor)
        ) {
    ...
    {
        // Retrieve user type and job by ID
        ...
        // Update job address, instructions, and town if provided in data, otherwise use user's details
        ...
        // Determine the email and name for sending the notification
            ...
            if (
                ...
                foreach ($jobs as $oneJob) {
                    if ($job->id == $oneJob->id) {
                        ...
                        $userId = $oneUser->id;
                        $job_for_translator = $this->model->assignedToPaticularTranslator(
                            $userId,
                            $oneJob->id
                        );
                        if ($job_for_translator == "SpecificJob") {
                            $job_checker = $this->model->checkParticularJob(
                                $userId,
                                $oneJob
                            );
    ...
    public function sendPushNotificationToSpecificUsers(
        ...
        $this->logger->addInfo("Push send for job " . $job_id, [
            $users,
            $data,
            $msg_text,
            $is_need_delay,
        ...
        $onesignalAppID = config(
            "app." .
                (env("APP_ENV") == "prod" ? "prod" : "dev") .
                "OnesignalAppID"
        );
        ...
        $this->logger->addInfo(
            "Push send for job " . $job_id . " curl answer",
            [$response]
        );
        curl_close($ch);
    }
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        $translatorsId = UsersBlacklist::where("user_id", $job->user_id)
            ->pluck("translator_id")
            ->all();
    ...
    protected function getTranslatorType($job_type)
    {
        switch ($job_type) {
            case "paid":
                return "professional";
    ...
    protected function getTranslatorLevels($certified)
    {
        $translator_levels = [];
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        $this->logJobUpdate($cuser, $job, $id, $log_data);
    ...
    protected function handleJobUpdates(
        $job,
        $changeDue,
        $changeTranslator,
        $langChanged
        ...
        if ($changeTranslator["translatorChanged"]) {
            $this->sendChangedTranslatorNotification(
                $job,
                $changeTranslator["current_translator"],
    ...
    {
        // Get admin email and sender email from configuration
        $adminEmail = config("app.admin_email");
        $adminSenderEmail = config("app.admin_sender_email");

        // Store the current user in a variable for readability
        $currentUser = $user;

        // Get the job ID from the data array
        $jobId = $data["job_id"];

        // Find the job by its ID or throw an exception if not found
        $job = $this->model->findOrFail($jobId);

        // Check if the user is already booked for another job at the same time
        if (
            !$this->model->isTranslatorAlreadyBooked(
                $jobId,
                $currentUser->id,
                $job->due
            )
            ...
            if (
                $job->status === "pending" &&
                $this->model->insertTranslatorJobRel($currentUser->id, $jobId)
            ) {
                ...
                // Determine the email address and name for the recipient
                $recipientEmail = !empty($job->user_email)
                    ? $job->user_email
                    : $jobUser->email;
                $recipientName = $jobUser->name;
                ...
                // Prepare a failure response if the job couldn't be assigned
                throw new Exception("Job assignment failed.");
            }
        } else {
            // Prepare a failure response if the user is already booked
            throw new Exception(
                "User is already booked for another job at the same time."
            );
        }
    ...
    public function acceptJobWithId($job_id, $cuser)
    {
        ...
        if (
            ...
            if (
                $job->status == "pending" &&
                $this->model->insertTranslatorJobRel($cuser->id, $job_id)
            ) {
                // Update the job status to 'assigned'
                $job->status = "assigned";
                $job->save();
                ...
                throw new Exception(
                    "Denna " .
                        ...
                        "min " .
                        $job->due .
                        " har redan accepterats av en annan tolk. Du har inte fått denna tolkning"
                );
            }
        } else {
    ...
    public function cancelJobAjax($data, $user)
    {
        ...
        if ($cuser->is("customer")) {
            ...
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                // Event::fire(new JobWasCanceled($job)); // Uncomment if needed
    ...
    public function getPotentialJobs($cuser)
    {
        ...
        foreach ($job_ids as $k => $job) {
            $jobuserid = $job->user_id;
            $job->specific_job = $this->model->assignedToPaticularTranslator(
                $cuser->id,
                $job->id
            );
            $job->check_particular_job = $this->model->checkParticularJob(
                $cuser->id,
                $job
            );
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser->user_type == env("SUPERADMIN_ROLE_ID")) {
            $this->applyFiltersForSuperadmin($allJobs, $requestdata);
            ...
            $this->applyFiltersForNonSuperadmin(
                $allJobs,
                $requestdata,
                $consumer_type
            );
    ...
    private function applyFiltersForSuperadmin($query, $filters)
    {
        ...
        if (isset($filters["lang"]) && $filters["lang"] != "") {
            $query->whereIn("from_language_id", (array) $filters["lang"]);
        }
        ...
        if (
            isset($filters["will_expire_at"]) &&
            $filters["will_expire_at"] != ""
        ) {
            $query->where("will_expire_at", ">=", $filters["will_expire_at"]);
        }

        if (
            isset($filters["customer_email"]) &&
            count($filters["customer_email"]) &&
            $filters["customer_email"] != ""
        ) {
            $userIds = DB::table("users")
                ->whereIn("email", $filters["customer_email"])
                ->pluck("id");
            $query->whereIn("user_id", $userIds);
        }

        if (
            isset($filters["translator_email"]) &&
            count($filters["translator_email"])
        ) {
            $userIds = DB::table("users")
                ->whereIn("email", $filters["translator_email"])
                ->pluck("id");
        ...
        if (
            isset($filters["filter_timetype"]) &&
            $filters["filter_timetype"] == "created"
        ) {
        ...
        if (isset($filters["job_type"]) && $filters["job_type"] != "") {
            $query->whereIn("job_type", (array) $filters["job_type"]);
        }

        if (isset($filters["physical"])) {
            $query
                ->where("customer_physical_type", $filters["physical"])
                ->where("ignore_physical_phone", 0);
        }

        if (isset($filters["phone"])) {
            $query
                ->where("customer_phone_type", $filters["phone"])
                ->where("ignore_physical_phone", 0);
        }
        ...
        if (
            isset($filters["consumer_type"]) &&
            $filters["consumer_type"] != ""
        ) {
    ...
    private function applyFiltersForNonSuperadmin(
        $query,
        $filters,
        $consumer_type
    ) {
        ...
        // Apply other filters specific to non-superadmin user
        // Use the $query variable to build the query with filters
    }
    ...
    public function reopen($request)
    {
        ...
        $data = [
            "created_at" => now(),
            "will_expire_at" => TeHelper::willExpireAt($jobArray["due"], now()),
            "updated_at" => now(),
            "user_id" => $userid,
            "job_id" => $jobid,
        ...
        if ($jobArray["status"] == "timedout") {
            ...
            $jobArray["will_expire_at"] = TeHelper::willExpireAt(
                $jobArray["due"],
                now()
            );
            $jobArray["cust_16_hour_email"] = 0;
            $jobArray["cust_48_hour_email"] = 0;
            $jobArray["admin_comments"] =
                "This booking is a reopening of booking #" . $jobid;
            $job = $this->model->create($jobArray);
            $new_jobid = $job->id;
        } else {
            $this->model->where("id", $jobid)->update($datareopen);
            $new_jobid = $jobid;
        }
    ...
    public function updateJobAttributes(
        $jobid,
        $admincomment,
        $session,
        $flagged,
